...
Two channels of exposure follow from it, and they are not the same. An engineer from a Central Asian jurisdiction employed by an American firm, and a researcher from the same jurisdiction holding credentials for the model, fell directly within the directive's stated criterion: they are foreign nationals, and the criterion names foreign nationals. A bank in that jurisdiction running inference against the same models would have lost access through a different route. It is a legal person, not a national of anywhere. It lost access because Anthropic, unable to verify the nationality of every user in real time, disabled the models for all customers in order to comply.[1] Direct criterion exposure and compliance spillover are distinct mechanisms with a common origin, and an institution mapping its own risk needs to know which one it is subject to.

In neither case was a decision taken about that jurisdiction, or any jurisdiction. No such decision was required. The mechanism did not pass through country-level classification at all.
...
It also carries a cost that must be stated plainly for the institutions this analysis addresses. For a bank in the region operating under secondary-sanctions exposure, adopting infrastructure or model provenance associated with a jurisdiction under United States scrutiny is itself a compliance event, to be assessed against the same risk register that governs correspondent relationships.
...
Whether regulatory instruments in the region — central bank, ministerial, or legislative — come to name foreign-provider access continuity as a documented risk category, in the way current frameworks already name cybersecurity and data localization.
...
[18] Khodjaev, O. *The Verification Ceiling: What Geneva Confirmed About Global AI Governance.* Analysis, 9 July 2026. [okhodjaev.com](https://okhodjaev.com/analysis/the-verification-ceiling/) See also *Governance Briefs,* Analytical Note No. 10, "The Alignment Ceiling." June 2026. [okhodjaev.com](https://okhodjaev.com/governance-briefs/the-alignment-ceiling/)
